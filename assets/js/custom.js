...
function typeEffect(el, speed = 40) {
...
function runTyping(scope) {
  scope.querySelectorAll(".typing").forEach((el) => {
    typeEffect(el);
  });
}
...
$(".owl-banner").on("changed.owl.carousel", function (event) {
  ...
  const slide = document.querySelectorAll(".owl-banner .item")[current];
  runTyping(slide);
});
